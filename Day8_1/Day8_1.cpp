...
int main()
{
    ...
    std::sort(groups.begin(), groups.end(), [](const std::vector<int>& a, const std::vector<int>& b) {return a.size() > b.size(); });
    std::cout << groups[0].size() * groups[1].size() * groups[2].size() << std::endl;
    return 0;
}
